    ...
    public class RolController : Controller
    {
        ...
        public async Task<IActionResult> Index(Rol pRol = null)
        {
            ...
            if (pRol.Top_Aux == 0)
                ...
                pRol.Top_Aux = 0;
            var roles = await rolBL.Buscar(pRol);
        ...
        {
            var rol = await rolBL.ObtenerPorId(new Rol { Id = id });
            return View(rol);
        }
        ...
        public async Task<IActionResult> Create(Rol pRol)
        {
            try
            {
                int result = await rolBL.Crear(pRol);
        ...
        {
            var rol = await rolBL.ObtenerPorId(pRol);
            ViewBag.Error = "";
            return View(rol);
        }
        ...
        public async Task<IActionResult> Edit(int id, Rol pRol)
        {
            try
            {
                int result = await rolBL.Modificar(pRol);
                return RedirectToAction(nameof(Index));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Delete(int id, Rol pRol)
        {
            try
            {
                int result = await rolBL.Eliminar(pRol);
                return RedirectToAction(nameof(Index));
            }
            catch (Exception ex)
            {
